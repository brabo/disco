...
#define STMPE811_GPIO_INT_EN	0x0C	/* GPIO interrupt enable register */
...
static void stmpe811_enable_ts(void);
static void stmpe811_disable_ts(void);
static void stmpe811_enable_gpio(void);
static void stmpe811_disable_gpio(void);
static void stmpe811_enable_tsc(void);
static void stmpe811_disable_tsc(void);
static void stmpe811_enable_adc(void);
static void stmpe811_disable_adc(void);
static void stmpe811_enable_fifo_oflow(void);
...
static void stmpe811_enable_gpio(void);
static void stmpe811_temp_start(void);
...
static void stmpe811_reset(void)
{
	...
	data |= STMPE811_SYS_CTRL1_SOFT_RESET;
...
static void stmpe811_enable_fifo_oflow(void)
{
	...
	mode |= STMPE811_INT_EN_FIFO_OFLOW;
...
static void stmpe811_enable_fifo_touch_det(void)
{
	uint8_t mode;
	mode = stmpe811_read(STMPE811_INT_EN);
	mode |= STMPE811_INT_EN_TOUCH_DET;
	stmpe811_write(STMPE811_INT_EN, mode);
}
...
static void stmpe811_reset_fifo(void)
{
	...
	sta |= STMPE811_FIFO_STA_TOUCH_DET;
...
static void stmpe811_enable_interrupts(void)
{
	...
	mode |= STMPE811_INT_CTRL_GLOBAL_INT;
...
stmpe811_state_t stmpe811_init(void)
{
	...
	stmpe811_set_tsc_fraction_z(0x07); /* set fractional part to 7 */

	stmpe811_set_tsc_i_drive(0x01);    /* set current limit to 50 mA */
	...
	//int i;
	//for ( i = 0; i < 0x200; i++) {
	//	uint32_t data = stmpe811_read(i);
	//	printf("0x%02X == 0x%08X\r\n", i, data);
	//}
	stmpe811_enable_gpio();
